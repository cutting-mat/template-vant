<template>
  <div class="workPage scrollbar">
    <div class="teamInfo">
      <div class="channelTitle">走：参观考察一个红色景点</div>
      <div class="p">
        织开展缅怀革命先烈、走访红色革命根据地，实地学习党的历史，在红色景点拍摄一组照片并打卡记录。
      </div>
    </div>
    <van-form ref="theForm" class="theForm" @submit="doSubmit()">
      <div class="teamInfo">
        ...
        >
          ...
          <template #button>
            <div class="_link" @click="canEdit && (showMapPicker = true)">
              {{ gpsLoading ? "定位中" : "定位成功" }}
              <van-icon name="arrow" />
            </div>
          </template>
      ...
      <div class="teamInfo">
        ...
        <!-- 上传文件展示 -->
        <FileList
          v-if="Array.isArray(editForm.filesExt)"
...
export default {
  ...
  computed: {
    canEdit() {
      // 非app端不能编辑
      return (
        String(this.$route.query.canEdit) === "true" &&
        !!util.storage("is_webview")
      );
    },
  },
  watch: {
  ...
  methods: {
    ...
    doSubmit() {
      ...
      this.loading = true;
      const actionApi = this.editForm.id ? api.editwalk : api.addwalk;
      // 更新gps
      if (this.currentGps) {
        this.editForm.gps = this.currentGps;
      }
...
.channelTitle::after {
  ...
  width: 20px;
  height: 20px;
  background: url(../../team/assets/img/work/work2.png) center center no-repeat
    #fe664e;
