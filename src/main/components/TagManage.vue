  ...
  <div class="tags flex-row" v-if="this.list.length">
    <div class="flex-1 tagsWrap">
      <van-tag
        ...
        >{{ (item.meta && item.meta.title) || item.name }}</van-tag
      >
    </div>
    <!-- 其他操作按钮 -->
    <van-dropdown @command="handleTags">
      <van-button type="text">
        <i class="el-icon-arrow-down"></i>
      </van-button>
      <van-dropdown-menu slot="dropdown">
        <van-dropdown-item command="other">关闭其他</van-dropdown-item>
        <van-dropdown-item command="all">关闭所有</van-dropdown-item>
      </van-dropdown-menu>
    </van-dropdown>
  </div>
</template>
