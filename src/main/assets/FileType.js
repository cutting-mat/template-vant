...
// 根据accept格式字符串获取扩展名
export const getExtByAccept = (acceptString) => {
    if (acceptString && acceptString.split) {
        const typeStringArray = acceptString.replace(/\./g, "").split(",");
        let result = [];
        typeStringArray.forEach((type) => {
            result = result.concat(getExtByType(type));
        });
        return result;
    } else {
        console.warn('getExtByAccept()参数异常')
    }
}
